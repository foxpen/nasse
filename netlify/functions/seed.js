...
export async function handler(event) {
  try {
    ...
    if (existing[0].c > 0 && !force) {
      return json(200, { ok: true, skipped: true, message: 'DB už obsahuje data (přidej ?force=1 pro doplnění)' });
    }
    ...
    return json(200, { ok: true, inserted: { byd: PROPS.length, auto: CARS.length } });
  } catch (e) {
